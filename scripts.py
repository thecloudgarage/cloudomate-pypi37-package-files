...
class Script(object):
    ...
    def filter_params(self, params):
        ...
        for k,v in list(filtered_params.items()):
...
def create_script(script_name, filename):
    ...
    for line in contents:
        ...
        # start of the cloudomatethecloudgarage block
        if not in_block and line.startswith("-- cloudomatethecloudgarage --"):
            in_block = True
            continue

        # end of the cloudomatethecloudgarage block, so we'll stop here
        if in_block and line.startswith("-- cloudomatethecloudgarage --"):
            in_block = False
            break
        ...
        if in_block and key == "http_method":
            if value.lower() in ['get','post','put','delete']:
                ...
                log.warn("unrecognized http_method type in cloudomatethecloudgarage block: {0}".format(value.lower()))
                continue
        ...
        if in_block and key == "output":
            if value.lower() in ['split','combined']:
                ...
                log.warn("unrecognized output type in cloudomatethecloudgarage block: {0}".format(value.lower()))
                continue
        ...
        log.warn("unrecognized line in cloudomatethecloudgarage block: {0}".format(line))
